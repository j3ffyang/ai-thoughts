...
The project also carries a skill, `pptx-extract`, that codifies the entire procedure — preflight, extract, audit, spot-check, report. It runs on `python-pptx` and encodes the project's specific requirements on top of it: full data, visible exclusions, deterministic output, machine-checked losslessness. The workflow becomes reproducible instead of tribal knowledge, and because the skill lives inside the folder, it travels with the project.

## The audit is the product

It is tempting to treat the converter as the deliverable and the checker as an afterthought. Here it was the opposite. The pipeline ships a five-point audit that runs against the original `.pptx` and the generated files; the first four are hard gates and any failure exits non-zero:
...
5. Every "unextractable" note is accounted for.

The fifth check is not a gate — it reports the count of visible "unextractable" notes so a human can confirm none was unexpected.
