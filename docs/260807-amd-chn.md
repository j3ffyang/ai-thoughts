...
**原文：** [260807-amd.md](260807-amd.md)

![fastfetch](../imgs/260807-2025.png)

## 文档目标

针对 GPD Win4 的两块 AMD GPU（iGPU + eGPU）在 Arch Linux + hyprland.lua 上的使用而写——将渲染任务卸载到 eGPU——但该方法具有通用性，因此也可作为其他硬件上外接 GPU（eGPU）方案的参考。
...
> 文档目标同时充当文档结构：每条要点对应下方的一个章节，按阅读顺序排列。
...
`~/.bashrc` 中有一个 `egpu()` 启动器——仅对 GL 卸载渲染，并硬编码了 PCI id：
...
1. **调用时动态解析 PCI id。** `lspci` 输出的 `03:00.0` 带点号——需替换 `[.:]`，而不只是 `:`。
...
```
   ...
   优点：没有过期 id，报错清晰（"not detected"）。缺点：每次运行都会多一次 `lspci` 调用。

2. **Vulkan 卸载。** `MESA_VK_DEVICE_SELECT=1002:7480`（已安装 layer）。已验证：会把 eGPU 重新排序到 GPU0，但**不会**隐藏另一块 GPU。对默认选用第一个适配器的应用有帮助；按名称选择的应用不受影响。

## 悬而未决的问题（已定论）

- **`03:00.0` 稳定吗？** 在所有接入 eGPU 的启动中均稳定（5/5）；未接 eGPU 时 iGPU 会移到 `63:00.0`。动态解析仍作为安全默认方案保留。
- **把 `MESA_VK_DEVICE_SELECT` 放进 `egpu()`？** 可选的加分项——只对没有内置显卡选择器的 Vulkan 游戏有帮助。非必需。
- **重命名 `egpu`？** 纯外观问题；保留即可。
- **启用 `AQ_DRM_DEVICES`？** 不——一切正常，而且 `igpu` 符号链接在未接 eGPU 的启动中会悬空，启用它只会增加风险而没有任何当前收益。
